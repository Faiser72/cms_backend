package com.vetologic.cms.model.dao.prescription;
...
public class PrescriptionDaoImpl implements PrescriptionDao {
	...
	public List<?> getAllByPatientId(String beanClassName, int id) {
		...
		try {
			Query<?> query = session
					.createQuery("FROM " + beanClassName + " WHERE activeFlag = ?0 AND patient.patientId = ?1");
			query.setParameter(0, 1);
	...
	public Object getByPatientId(String beanClassName, int id) {
		...
		Object object = null;
		try {
			Query<?> query = session.createQuery("FROM " + beanClassName + " WHERE activeFlag = ?0 AND patient = ?1");
			query.setParameter(0, 1);
			query.setParameter(1, id);
			object = query.uniqueResult();
		} catch (Exception e) {
